...
<div class="flex h-screen w-screen items-center justify-center bg-void-bg-primary">
  <div class="w-full max-w-md space-y-8 px-6">
    ...
    <div class="text-center">
      <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-void-accent/20">
        <svg class="h-8 w-8 text-void-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    {:else if auth.polling}
      ...
      <div class="space-y-4 rounded-xl border border-void-border bg-void-bg-secondary p-6 text-center">
        ...
        <p class="text-sm text-void-text-secondary">Waiting for GitHub authorization...</p>
        <p class="text-xs text-void-text-muted">Complete the authorization in your browser</p>
      </div>
    {:else}
      <!-- Login button -->
      <div class="space-y-4">
        ...
        <p class="text-center text-xs text-void-text-muted">
          We use GitHub's Device Flow — no passwords stored locally.
        </p>
      </div>
    {/if}
  </div>
