...
- **Server-mode screen share media path + in-app diagnostics** (`frontend/src/lib/services/voiceRTC.ts`, `frontend/src/lib/stores/voice.svelte.ts`, `frontend/src/lib/components/voice/VoiceControls.svelte`, `frontend/src/lib/components/voice/ScreenShareTile.svelte`, `frontend/src/App.svelte`, `frontend/src/lib/components/layout/{ChannelSidebar,DMSidebar}.svelte`): screen sharing now adds/removes real WebRTC tracks per peer (audio/video), synchronizes local/remote screen-share status, and exposes live connection diagnostics inside the app UI (WebSocket state, reconnect attempts, peer stats, recent RTC events) so QA can debug without terminal logs.
- **Signaling peer state expansion** (`internal/network/signaling/signaling.go`, `internal/network/signaling/server.go`): `screen_sharing` was added to join/peer-state/peer-list payloads and API-backed peer entries, keeping channel participant badges and live-state polling consistent with WebRTC state.
- **Dominant speaker + per-peer media quality** (`frontend/src/lib/services/voiceRTC.ts`, `frontend/src/lib/stores/voice.svelte.ts`, `frontend/src/lib/components/layout/ChannelSidebar.svelte`): voice participants now carry real RMS volume updates, dominant speaker tagging, connection-quality grading (`good/fair/poor/unknown`), and numeric quality score derived from ICE/RTC stats (RTT, jitter, loss), with sidebar badges for quick operator visibility during calls.
- **Guided troubleshooting inside voice panel** (`frontend/src/lib/components/voice/VoiceControls.svelte`): diagnostics mode now includes automatic hints for signaling disconnects, reconnect storms, ICE failures, poor peer quality, and autoplay blocks, reducing dependence on external logs during app-based testing.
- **Adaptive screen-share QoS** (`frontend/src/lib/services/voiceRTC.ts`): when screen sharing is active, WebRTC stats now drive dynamic video constraints (`high/balanced/low` profiles with cooldown), reducing frame rate/resolution for degraded peers and recovering quality when links stabilize.
- **Automatic ICE restart recovery** (`frontend/src/lib/services/voiceRTC.ts`): peers in `disconnected/failed` states now trigger bounded ICE restart attempts (cooldown + max tries) before forced peer removal, improving recovery from transient network failures without manual rejoin.
- **Proactive poor-quality alerts** (`frontend/src/lib/stores/voice.svelte.ts`): the client now emits throttled in-app warning toasts when a remote peer transitions into `poor` connection quality, making call degradation visible even without diagnostics panel open.

### Fixed

- **Voice negotiation deadlock — zero `sdp_answer` ever sent** (`frontend/src/lib/services/voiceRTC.ts`): replaced MDN Perfect Negotiation pattern with Jitsi-style role-based negotiation. Root cause: `peer.ignoreOffer` was set `true` during offer collision but never reset, permanently blocking answers and ICE candidates. New architecture: joiner (peer_list receiver) is always the initiator, existing peer (peer_joined receiver) is always the responder. Responders suppress `onnegotiationneeded` and only create answers. Glare is now impossible by design.
...
- **`sdp_answer` missing under concurrent join signaling** (`frontend/src/lib/services/voiceRTC.ts`): signaling message handling is now serialized, existing peers wait for joiner-initiated offer (`peer_joined` no longer auto-offers), and remote ICE candidates are buffered until `remoteDescription` is set, preventing negotiation deadlocks and early-candidate drops.
...
- **Broken server-mode RTC runtime** (`frontend/src/lib/services/voiceRTC.ts`): restored the full `VoiceRTCClient` implementation after a truncated file state that left only interfaces and caused voice/session flows to fail at runtime.
...
- **Friends real-time UX and presence** (`frontend/src/lib/stores/friends.svelte.ts`, `frontend/src/lib/api/friends.ts`, `internal/friends/repository.go`): reduced friends/pending polling interval to near real-time, added incoming friend-request notifications, prevented self friend-request in frontend, synced DM status metadata continuously, and now resolves friend online/offline status from real-time presence tracking.
- **DM notification behavior** (`frontend/src/lib/stores/friends.svelte.ts`, `frontend/src/lib/services/notifications.ts`): incoming DM messages now increment unread counters and trigger desktop/toast notifications when the user is not viewing that DM conversation.
- **Voice connected panel cleanup** (`frontend/src/lib/components/voice/VoiceControls.svelte`, `DMSidebar.svelte`, `ChannelSidebar.svelte`): removed participant list under “voice connected”; participants remain visible only inside voice channel lists.
- **WebRTC voice stability** (`frontend/src/lib/services/voiceRTC.ts`): added pending ICE candidate queue/flush to handle early candidates, improved transient `disconnected` handling with grace timeout before peer removal, and tightened peer cleanup to reduce join/leave churn and one-way-audio failures.
- **TURN relay support for voice in production** (`internal/voice/ice_config.go`, `internal/api/handlers_voice.go`, `cmd/server/main.go`, `frontend/src/lib/services/voiceRTC.ts`, `deployments/docker/docker-compose.prod.yml`): added authenticated `GET /api/v1/voice/ice-config` endpoint with ephemeral TURN REST credentials, frontend ICE auto-discovery before peer connection, and optional coturn service/env wiring for NAT-safe audio delivery.
...
- **Signaling sender identity stamping** (`internal/network/signaling/server.go`, `frontend/src/lib/services/voiceRTC.ts`): SDP/ICE forwarding now always carries canonical `from` peer IDs from the active websocket session, fixing negotiation drops where peers could not hear each other.
- **Remote speaking indicator** (`voiceRTC.ts`): added per-peer audio analyzer loop that updates `speaking` state in near real-time, so users in the same voice channel now see each other’s voice activity.
...
- **Voice mute/deafen state + timer sync** (`internal/network/signaling/signaling.go`, `internal/network/signaling/server.go`, `frontend/src/lib/services/voiceRTC.ts`, `frontend/src/lib/stores/voice.svelte.ts`, `ChannelSidebar.svelte`): broadcast per-peer mute/deafen state and include channel start timestamps so all participants see consistent voice timers and mute/deafen indicators.
...
- **Friend online status accuracy** (`internal/presence/presence.go`, `internal/api/middleware.go`, `internal/friends/service.go`, `cmd/server/main.go`): online/offline now reflects recent activity instead of long-lived auth sessions.
- **False positive friend online state** (`internal/friends/repository.go`, `cmd/server/main.go`, `frontend/src/lib/stores/friends.svelte.ts`): removed `auth_sessions` online fallback from friends query, reduced in-memory presence TTL to `15s`, and added stale-cache/poll-failure fallback to force cached presence to `offline` when real-time sync is not fresh.
- **Immediate offline update on exit/logout** (`internal/presence/presence.go`, `internal/api/server.go`, `internal/api/handlers_presence.go`, `frontend/src/lib/stores/auth.svelte.ts`, `frontend/src/App.svelte`, tests in `internal/presence/presence_test.go` and `internal/api/handlers_presence_test.go`): added protected `POST /api/v1/presence/offline`, backend `SetOffline` tracker operation, and frontend best-effort presence-offline signaling on logout and `beforeunload` so friends see offline status without waiting for TTL expiry.
- **Stale presence ghost online** (`internal/presence/presence.go`, `internal/api/handlers_presence.go`, `internal/api/server.go`, `frontend/src/lib/stores/auth.svelte.ts`): presence tracker now runs a background reaper goroutine that evicts stale entries every TTL/2, preventing users who crash/disconnect without clean shutdown from staying online in friends list. Offline endpoint now also accepts `?token=` query param for `navigator.sendBeacon` fallback (sendBeacon cannot set custom headers), making page-unload offline signaling much more reliable.
- **Voice channels no audio between peers** (`frontend/src/lib/services/voiceRTC.ts`, `internal/voice/engine.go`, `internal/api/handlers_voice.go`): added free public TURN relay servers as fallback ICE configuration when no dedicated TURN is configured, enabling NAT traversal for peers behind symmetric NATs. Also fixed audio autoplay blocking by re-assigning srcObject on track arrival and adding user-interaction resume handlers, and added ICE connection diagnostics logging.
