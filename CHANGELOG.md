...
- **False positive friend online state** (`internal/friends/repository.go`, `cmd/server/main.go`): removed `auth_sessions` online fallback from friends query and reduced in-memory presence TTL to `30s`, so offline users stop appearing online quickly when they disconnect.
