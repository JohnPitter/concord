...
- **Friends real-time UX and presence** (`frontend/src/lib/stores/friends.svelte.ts`, `frontend/src/lib/api/friends.ts`, `internal/friends/repository.go`): reduced friends/pending polling interval to near real-time, added incoming friend-request notifications, prevented self friend-request in frontend, synced DM status metadata continuously, and now resolves friend online/offline status from active auth sessions instead of hardcoded `offline`.
